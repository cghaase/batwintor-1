...
```
## Model Environment
In order to run the model we need to create an environmental space across which to model. This can be a major determinate for how long the model runs. The larger the environmental space, and the longer the time of winter, the more model runs are required and that can potentially take a very long time depending upon the computers performance.  
```{r}
...
```
## Concluding Remarks
Over all that didn't take too long, but as either ranges for the variables, or the resolution of the data set increase the calculations can take a decent amount of time so keep that in mind. This concludes this vignette, and these results will be used to continue into the plotting vignette. 
