...
coolEnergy<- function(Ta, bat.params){
  with(bat.params,{
    ...
    ifelse(Ta > Ttormin,
           (TMRmin + (RMR*Q^((Ta-Teu)/10)))*((Teu-Ta)/((Ceu*((10*(Mass^0.67)) + SA.wing)*(Teu-Ta))/(0.1728*Mass))),
           (TMRmin + (RMR*Q^((Ttormin-Teu)/10)))*((Teu-Ttormin)/((Ceu*((10*(Mass^0.67)) + SA.wing)*(Teu-Ta))/(0.1728*Mass)))
    )
