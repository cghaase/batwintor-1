#' Metabolic paramerters for 6 species of hibernating bats.
#'
#' A dataset containing information on euthermic and torpid metabolic rates of
#' 6 species of hibernating bats.
#'
#' @format a dataset containg 14 measurements for 6 species
#' \describe{
#'   \item{RMR}{esting metabolic rate (TODO Units)}
#'   \item{TMRmin}{minimum metabolic rate muring torpor (TODO units)}
...
"bat.params"
bats <- c("M.lucifigus", "M.myotis", "E.serotinus", "E.fuscus", "M.yumanensis",
  "M.californicus")
...
WR <- rep(NA, 6)
CR <- rep(NA, 6)
bat.params <- cbind(RMR = RMR, TMRmin = TMRmin, Teu = Teu, Tlc = Tlc,
  Ttormin = Ttormin, Ceu = Ceu, Ct = Ct, S = S, ttormax = ttormax, tar = tar,
  teu = teu, mass = mass, WR = WR, CR = CR)
